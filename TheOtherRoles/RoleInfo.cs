...
namespace TheOtherRoles
{
    class RoleInfo {
        ...
        public static List<RoleInfo> getRoleInfoForPlayer(PlayerControl p) {
            ...
            if (p == Seer.player) infos.Add(seer);
            if (p == Hacker.player) infos.Add(hacker);
            if (p == Mini.player) infos.Add(p.Data.Role.IsImpostor ? evilMini : niceMini);
            ...
            if (p == SecurityGuard.player) infos.Add(securityGuard);
            if (p == Arsonist.player) infos.Add(arsonist);
            if (p == Guesser.player) infos.Add(p.Data.Role.IsImpostor ? badGuesser : goodGuesser);
            if (p == BountyHunter.player) infos.Add(bountyHunter);
            if (p == Bait.player) infos.Add(bait);
            if (p == Vulture.player) infos.Add(vulture);
            if (p == Medium.player) infos.Add(medium);
            if (p == Lawyer.player) infos.Add(lawyer);
